...
export const STARTER_LAND_COUNT = 8;
export const LAND_UNLOCK_ORDER = [
    0, 1, 7, 8,
    14, 15, 21, 22,
    2, 9, 16, 3,
    10, 17, 4, 11,
...
export function createDefaultPlot(id) {
    return {
        id,
        state: 'empty',
        cropType: null,
...
export function clearPlot(plot, id = plot?.id || 0) {
    return {
        id,
        state: 'empty',
        cropType: null,
...
export function getNextLandPlotId(land = {}) {
    const count = clampUnlockedLandCount(land.unlockedCount);
    return count >= MAX_PLOTS ? null : LAND_UNLOCK_ORDER[count];
}

export function getLandPurchasePrice(landOrUnlockedCount = STARTER_LAND_COUNT) {
...
export function getLandRequiredLevel(landOrUnlockedCount = STARTER_LAND_COUNT) {
    const unlockedCount = typeof landOrUnlockedCount === 'object'
        ? clampUnlockedLandCount(landOrUnlockedCount?.unlockedCount)
        : clampUnlockedLandCount(landOrUnlockedCount);
...
export function normalizePlotsForLand(plots = [], land = {}, { clearLocked = true } = {}) {
